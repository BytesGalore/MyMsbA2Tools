...
int serial_handler::open_serial_port(const char *port_name)
{
	baud_rate = "115200";
	port_fd = -1;
	...
	r = set_baud(baud_rate.c_str());
	if (r == 0) {
		printf("Port \"%s\" (%d) opened at %s baud\r\n",
			port_name, port_fd, baud_rate.c_str());
	} else {
		printf("Port \"%s\" opened, unable to set baud to %s\r\n",
			port_name, baud_rate.c_str());
	}
	#ifdef LINUX
	{
 ...
 */
void serial_handler::report_open_error(const char *filename, int err)
{
	struct stat info;
	uid_t my_uid;
	gid_t my_gid;
...
int serial_handler::write_serial_port(const void *buf, int num)
{
	return(write(port_fd, buf, num));
}


void serial_handler::input_flush_serial_port(void)
{
	tcflush(port_fd, TCIFLUSH);
}


int serial_handler::read_serial_port_nb(unsigned char *buf, int bufsize)
{
	int num, flags;
...
int serial_handler::read_serial_port(unsigned char *buf, int bufsize)
{
	int num;
        num = read(port_fd, buf, bufsize);
	return num;
...
void serial_handler::send_break_signal(void)
{
	tcsendbreak(port_fd, 0);
}


void serial_handler::close_serial_port(void)
{
	if (port_fd >= 0) {
		close(port_fd);
		port_fd = -1;
...
tcflag_t serial_handler::baud_name_to_flags(const char *baud_name)
{
...
int serial_handler::serial_port_fd(void)
{
	return port_fd;
}
...
void serial_handler::set_rts(int val)
{
	int flags;
	int result;
...
void serial_handler::set_dtr(int val)
{
	int flags;
	int result;
